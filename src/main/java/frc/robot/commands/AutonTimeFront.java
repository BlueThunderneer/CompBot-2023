...
public class AutonTimeFront extends SequentialCommandGroup {
  /**
   ...
   * 
   */
  public AutonTimeFront(ClawSS m_ClawSS, DriveSubsystem m_drivetrain, ArmSS m_armss) {
       addCommands(
        new armdownTime(1.3, m_armss), // Lowers arm because yes
        //new armupTime(3.0, m_armss), //Raise the Cargo arm to shoot need to add this subsystem to the contstructor)
        new extendTime(2.5, m_armss),
        new ItsClawinTime(1.0, m_ClawSS), // Not a Morbius reference (Opens le claw)
        new retractTime(2.3, m_armss),
        //new Time(1),     //One Second Delay
        //new ArmdownTime(1.0, m_ArmSS),
        new armupTime(1.3, m_armss), //Raise the Cargo arm to shoot need to add this subsystem to the contstructor)
